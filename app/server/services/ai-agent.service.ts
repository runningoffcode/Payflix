...
import { Payment, Video } from '../types';
...
export class AIAgentService {
  ...
  async verifyAndSplitPayment(
    ...
    try {
      ...
      const isValid = await solanaService.verifyPayment(
      ...
      console.log(`  - Creator receives: ${split.creatorAmount} USDC (${config.fees.creatorPercentage}%)`);
      console.log(`  - Platform receives: ${split.platformAmount} USDC (${config.fees.platformPercentage}%)`);
  ...
  async monitorPayment(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        const tx = await solanaService.getTransaction(transactionSignature);
   ...
   */
  async calculatePlatformMetrics(payments: Payment[]): Promise<{
    totalRevenue: number;
