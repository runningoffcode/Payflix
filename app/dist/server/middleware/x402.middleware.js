...
exports.checkExistingPayment = checkExistingPayment;
 ...
 */
function requireX402Payment(getPaymentInfo) {
    return async (req, res, next) => {
        try {
            ...
            if (!paymentHeader) {
                ...
                const headers = x402_facilitator_service_1.x402Facilitator.createPaymentRequiredHeaders(paymentInfo.amount, paymentInfo.recipient, paymentInfo.resource);
                return res
                    .status(402)
                    .set(headers)
                    .json({
            ...
            const payload = x402_facilitator_service_1.x402Facilitator.parsePaymentHeader(paymentHeader);
            if (!payload) {
            ...
            const verification = await x402_facilitator_service_1.x402Facilitator.verifyPayment(payload);
            ...
            const settlement = await x402_facilitator_service_1.x402Facilitator.settlePayment(payload);
 ...
 */
async function recordPayment(req, transactionSignature, amount, creatorWallet) {
    ...
    // Get user
    const user = await db_factory_1.db.getUserByWallet(userWallet);
    if (!user) {
    ...
    // Record payment
    await db_factory_1.db.createPayment({
        videoId: video.id,
        userId: user.id,
        userWallet,
        creatorWallet,
...
function checkExistingPayment() {
    return async (req, res, next) => {
        try {
            ...
            const hasAccess = await db_factory_1.db.hasVideoAccess(user.id, videoId);
            if (hasAccess) {
