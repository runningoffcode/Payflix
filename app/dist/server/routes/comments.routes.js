...
const analytics_upsert_service_1 = require("../services/analytics-upsert.service");
...
router.post('/', async (req, res) => {
    try {
        ...
        if (effectivePrice > 0) {
            ...
            const paymentResult = await session_payment_service_1.sessionPaymentService.processSessionPayment({
                userWallet,
                videoId,
            ...
            const currentEarnings = typeof video.earnings === 'number' && !Number.isNaN(video.earnings) ? video.earnings : 0;
        ...
        const updatedBalance = effectivePrice > 0
            ? await session_payment_service_1.sessionPaymentService.getSessionBalance(userWallet)
            : null;
        ...
        await (0, analytics_upsert_service_1.recordVideoAnalyticsDelta)(videoId, commentDelta, analyticsDate);
        await (0, analytics_upsert_service_1.recordCreatorAnalyticsDelta)(video.creator_wallet, commentDelta, analyticsDate);
        if (video.creator_wallet) {
            (0, digital_id_routes_1.invalidateDigitalIdCache)(video.creator_wallet);
        }
...
router.delete('/:commentId', async (req, res) => {
    try {
      ...
      `)
        ...
        const isCreator = comment.videos?.users?.wallet_address === userWallet;
 ...
 */
router.put('/videos/:videoId/settings', async (req, res) => {
    try {
        const { videoId } = req.params;
        ...
        if (commentPrice !== undefined)
            updates.comment_price = commentPrice;
