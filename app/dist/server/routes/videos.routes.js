...
const x402_middleware_1 = require("../middleware/x402.middleware");
...
router.get('/', async (req, res) => {
    try {
        ...
        const normalizedCategory = typeof category === 'string' && category.trim().length > 0 && category.toLowerCase() !== 'all'
            ? category.trim()
            : undefined;
        const orderByParam = typeof sort === 'string' && ['created_at', 'views', 'price_usdc'].includes(sort)
            ? sort
            : 'created_at';
        const orderDirectionParam = typeof order === 'string' && order.toLowerCase() === 'asc' ? 'asc' : 'desc';
        const creatorWallet = typeof creatorWalletQuery === 'string' && creatorWalletQuery.trim().length > 0
            ? creatorWalletQuery.trim()
            : undefined;
        let videos;
        let total;
        if (typeof search === 'string' && search.trim().length > 0) {
            const result = await db_factory_1.db.searchVideos({
                search,
                limit: parsedLimit,
                offset,
            ...
            const allVideos = await db_factory_1.db.getAllVideos();
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const video = await db_factory_1.db.getVideoById(id);
        if (!video) {
 ...
 */
router.get('/:id/stream', (0, x402_middleware_1.checkExistingPayment)(), (0, x402_middleware_1.requireX402Payment)(async (req) => {
    const video = await db_factory_1.db.getVideoById(req.params.id);
    if (!video) {
 ...
 */
router.get('/:id/play-url', (0, x402_middleware_1.checkExistingPayment)(), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        let existingSession = await db_factory_1.db.getActiveStreamingSession(userWallet, id);
        let sessionToken;
        let sessionId;
        if (existingSession) {
            ...
            await db_factory_1.db.createStreamingSession({
                id: sessionId,
                userWallet,
                videoId: id,
                sessionToken,
        ...
        const signedUrl = await r2_storage_service_1.r2StorageService.getSignedUrl(video.videoPath, 3600);
 ...
 */
router.get('/:id/stream-secure', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const session = await db_factory_1.db.getStreamingSessionByToken(sessionToken);
        if (!session) {
        ...
        db_factory_1.db.updateStreamingSessionAccess(sessionToken).catch(err => console.error('Warning: Failed to update session access time:', err));
        // Get video details
        const video = await db_factory_1.db.getVideoById(id);
        if (!video) {
            return res.status(404).json({ error: 'Video not found' });
...
router.post('/', async (req, res) => {
    try {
        ...
        let creator = await db_factory_1.db.getUserByWallet(creatorWallet);
        if (!creator) {
            creator = await db_factory_1.db.createUser({
                walletAddress: creatorWallet,
                isCreator: true,
            });
        }
        const videoId = `video_${(0, uuid_1.v4)()}`;
...
router.put('/:id/edit', async (req, res) => {
    try {
        ...
        await db_factory_1.db.updateVideo(id, {
            title,
            description,
            priceUsdc: price_usdc,
        });
        // Update comment settings using Supabase
        await db_factory_1.db.upsertCommentSettings(id, comments_enabled, comment_price);
...
router.patch('/:id/archive', async (req, res) => {
    try {
        ...
        const updated = await db_factory_1.db.updateVideo(id, { archived });
        if (!updated) {
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const payments = await db_factory_1.db.getPaymentsByVideo(id);
        ...
        const deleted = await db_factory_1.db.deleteVideo(id);
        if (!deleted) {
...
router.get('/thumbnails/:fileId', async (req, res) => {
    try {
        ...
        const signedUrl = await r2_storage_service_1.r2StorageService.getSignedUrl(`thumbnails/${fileId}`, 3600);
...
router.patch('/:id/archive', async (req, res) => {
    try {
        ...
        await db_factory_1.db.updateVideo(id, { archived: true });
