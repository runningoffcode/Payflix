...
class SupabaseDatabase {
    ...
    async updateUser(id, updates) {
        ...
        if (updates.profilePictureUrl !== undefined)
            dbUpdates.profile_image_url = updates.profilePictureUrl;
        const { data, error } = await this.supabase
            .from('users')
            .update(dbUpdates)
            .eq('id', id)
    ...
    async searchVideos(params) {
        ...
        const sanitizeForLike = (value) => value.replace(/[%_]/g, (char) => `\\${char}`);
    ...
    async updateVideo(id, updates) {
        ...
        if (updates.priceUsdc !== undefined)
            dbUpdates.price_usdc = updates.priceUsdc;
        ...
        if (updates.videoPath !== undefined)
            dbUpdates.video_path = updates.videoPath;
        if (updates.duration !== undefined)
            dbUpdates.duration = updates.duration;
        if (updates.views !== undefined)
            dbUpdates.views = updates.views;
        if (updates.earnings !== undefined)
            dbUpdates.earnings = updates.earnings;
    ...
    async updatePayment(id, updates) {
        ...
        if (updates.verifiedAt !== undefined)
            dbUpdates.verified_at = updates.verifiedAt;
        const { data, error } = await this.supabase
            .from('payments')
            .update(dbUpdates)
            .eq('id', id)
    ...
    async updateSessionBalance(sessionId, newApprovedAmount, newRemainingAmount, approvalSignature, expiresAt) {
        const { error } = await this.supabase
            .from('sessions')
            .update({
    ...
    async getStreamingSessionByToken(sessionToken) {
        const { data, error } = await this.supabase
            .from('video_streaming_sessions')
            .select('*')
    ...
    async getActiveStreamingSession(userWallet, videoId) {
        const { data, error } = await this.supabase
            .from('video_streaming_sessions')
            .select('*')
            .eq('user_wallet', userWallet)
    ...
    async updateStreamingSessionAccess(sessionToken) {
        const { error } = await this.supabase
            .from('video_streaming_sessions')
            .update({
    ...
    async cleanupExpiredStreamingSessions() {
        const { data, error } = await this.supabase
            .from('video_streaming_sessions')
            .delete()
    ...
    // Comment Settings
    async upsertCommentSettings(videoId, commentsEnabled, commentPrice) {
        const { error } = await this.supabase
...
exports.db = new SupabaseDatabase();
