...
- **Creator telemetry** – deterministic analytics feeds surface views, subscribers, payouts (`docs/overview/platform.md`).
...
- **MCP automations** – partners can script unlocks, analytics, and payouts via JSON-RPC (`docs/mcp-tooling.md`).
...
React Mission Console ─┐
                       │  MCP JSON-RPC
Mission Console / Agent Clients ─┼────▶ PayFlix MCP Server (Express + MCP tooling)
                       │                    │
                       │                    │ X402 Session Service
                       │                    ▼
                    Supabase (DB/Auth/Storage) ─┬─ Telemetry pipelines
                                                └─ Arweave permanent storage
```

- **Frontend** (`src/`): Next/React mission console, MCP client hooks, Solana wallet adapters.
- **Backend** (`server/`): Express routes, MCP handlers, session-payment service, analytics + telemetry pipelines.
...
4. Corbits helper wraps fetch for partner facilitators (Appendix E in `docs/infra/auth.md`).
...
- Session balance + payout events logged for MCP agents and frontend overlays.

### PayFlix UI / Unlock Flow

- Mission console overlays show Digital IDs (“Verified 12s ago”), telemetry stats, and agent-ready MCP buttons (`docs/infra/ui.md`).
...
| Analytics & telemetry briefs                                                                               | [`docs/overview/platform.md`](docs/overview/platform.md)                                             |
...
- Partner facilitator proxy (`docs/facilitator-proxy.md`)

---

## 📣 Call to Action

- **MCP partner?** → [docs/mcp-partner-pack.md](docs/mcp-partner-pack.md)
...
Questions, demos, or access requests → ping the team (Slack/Discord) or open an issue. PayFlix is ready for production MCP agents, Solana X402 streaming commerce. 🚀
