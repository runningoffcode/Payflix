...
function SocialButton({ name, href, icon }: (typeof SOCIAL_LINKS)[number]) {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="group flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-medium uppercase tracking-[0.25em] text-white transition hover:border-flix-cyan/60 hover:bg-white/10"
    >
...
function ContractAddress() {
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className="group flex max-w-full items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-left transition hover:border-flix-cyan/60 hover:bg-white/10"
    >
      <div className="flex flex-col gap-1 overflow-hidden">
        <span className="text-[10px] uppercase tracking-[0.5em] text-white/60">Token CA</span>
        <span className="font-mono text-sm tracking-[0.3em] text-white/90">
          {PLACEHOLDER_CA}
        </span>
      </div>
      <div className="ml-auto flex flex-col items-center gap-1">
        <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-white transition group-hover:border-flix-cyan/60 group-hover:bg-flix-cyan/10">
          <svg
            className="h-4 w-4"
            viewBox="0 0 24 24"
            fill="none"
...
export default function Landing({ onEnter }: LandingProps) {
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-black text-white">
      <ShaderBackground />
      <div className="absolute inset-0 bg-gradient-to-br from-black/40 via-black/10 to-flix-purple/20" />

      <header className="relative z-20 flex items-center justify-between gap-6 px-6 py-6 sm:px-10">
        <div className="flex items-center gap-4">
          <img src="/payflix-official-logo.svg" alt="PayFlix icon" className="h-12 w-auto" />
          <img src="/payflix-text.svg" alt="PayFlix" className="h-7 w-auto" />
        </div>
      ...
      <main className="relative z-10 flex min-h-screen flex-col items-center justify-center px-6 text-center">
        <div className="flex flex-col items-center gap-6">
          ...
          <p className="max-w-xl text-base text-white/70 sm:text-lg">
            Wallet-connected streaming on Solana. Instant creator payouts, zero popups, and premium
            premieres when you’re ready.
          </p>
          <button
            type="button"
            onClick={onEnter}
            className="mt-4 inline-flex items-center gap-3 rounded-full bg-gradient-to-r from-solana-purple to-flix-cyan px-10 py-3 text-sm font-semibold uppercase tracking-[0.4em] text-black shadow-lg shadow-solana-purple/30 transition hover:scale-[1.02]"
          >
            Enter the Flix
        ...
        <div className="mt-12 flex flex-col items-center gap-2 text-xs text-white/50">
